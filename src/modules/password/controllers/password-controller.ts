...
export default class PasswordController {
  static async getUserPasswords(req: Request, res: Response) {
    try {
      ...
      Logger.INFO(
        PasswordController.name,
        PasswordController.getUserPasswords.name,
  ...
  static async getPasswordDetails(req: Request, res: Response) {
    try {
      Logger.INFO(
        PasswordController.name,
        PasswordController.getPasswordDetails.name,
        "Inside get user passwords"
      );
      let passwordId = req.params.id;
      let result = await PasswordModel.findByPk(passwordId);
      let passwordObj = result?.toJSON();
      if (passwordObj) {
        let cryptrObj = new Cryptr(process.env.TOKEN_KEY as string);
        passwordObj.plain_password = cryptrObj.decrypt(passwordObj.password);
      }
      res.status(200).json({
        data: passwordObj,
        message: "User passwords list fetched successfully",
  ...
  static async createUserPassword(req: Request, res: Response) {
    try {
      Logger.INFO(
        PasswordController.name,
        PasswordController.createUserPassword.name,
        "Inside create user password"
      );
      let reqBody = req.body;
      let passwordObj: IPasswordModel = {};
  ...
  static async updateUserPassword(req: Request, res: Response) {
    try {
      Logger.INFO(
        PasswordController.name,
        PasswordController.updateUserPassword.name,
        "Inside update user password"
      );
      ...
      let tokenData = req.tokenData as TokenData;
      passwordObj.user_id = tokenData.user_id;
      let newPassword = await PasswordService.updatePassword(
        passwordId,
        passwordObj
      );
      res.status(204).json({
  ...
  static async deleteUserPassword(req: Request, res: Response) {
    try {
      Logger.INFO(
        PasswordController.name,
        PasswordController.deleteUserPassword.name,
        "Inside update user password"
      );
      let passwordId = parseInt(req.params.id);
